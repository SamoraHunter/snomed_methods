...
class TestConceptMapper:
    ...
    def test_initialization_without_paths(self):
        ...
        from src.snomed_methods import ConceptMapper
    ...
    def test_load_icd_mapping_no_file(self):
        """Test loading ICD mapping when no file is available."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        result = mapper.load_icd_mapping()
        assert isinstance(result, dict)
        assert len(result) == 0

    def test_load_loinc_mapping_no_file(self):
        """Test loading LOINC mapping when no file is available."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        result = mapper.load_loinc_mapping()
        assert isinstance(result, dict)
        assert len(result) == 0

    def test_map_to_icd_no_mappings(self):
        """Test mapping for a CUI with no ICD mappings."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        result = mapper.map_to_icd("999999")
        assert isinstance(result, list)
        assert len(result) == 0

    def test_map_to_loinc_no_mappings(self):
        """Test mapping for a CUI with no LOINC mappings."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        result = mapper.map_to_loinc("999999")
        assert isinstance(result, list)
        assert len(result) == 0

    def test_map_to_rxnorm_no_medcat(self):
        """Test RxNorm mapping without MedCAT model pack."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        result = mapper.map_to_rxnorm("409681000000102")
        assert isinstance(result, list)

    def test_map_bidirectional_snomed(self):
        """Test bidirectional mapping from SNOMED."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        result = mapper.map_bidirectional("409681000000102", "SNOMED")
        assert isinstance(result, list)

    def test_map_bidirectional_icd10(self):
        """Test bidirectional mapping from ICD-10."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        result = mapper.map_bidirectional("J44.1", "ICD10")
        assert isinstance(result, list)
        assert len(result) > 0
        assert result[0]["target_vocab"] == "SNOMED (estimated)"

    def test_map_bidirectional_loinc(self):
        """Test bidirectional mapping from LOINC."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        result = mapper.map_bidirectional("718-7", "LOINC")
        assert isinstance(result, list)
        assert len(result) > 0
        assert result[0]["target_vocab"] == "SNOMED (estimated)"
    ...
    def test_export_mappings(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
            output_path = f.name
        try:
            count = mapper.export_mappings(["409681000000102"], output_path)
...
class TestConvenienceFunctions:
    ...
    def test_map_concept_icd(self):
        """Test map_concept function with ICD-10 target."""
        from src.snomed_methods import map_concept

        result = map_concept("409681000000102", "ICD-10")
        assert isinstance(result, list)

    def test_map_concept_loinc(self):
        """Test map_concept function with LOINC target."""
        from src.snomed_methods import map_concept

        result = map_concept("409681000000102", "LOINC")
        assert isinstance(result, list)

    def test_map_concept_rxnorm(self):
        """Test map_concept function with RxNorm target."""
        from src.snomed_methods import map_concept

        result = map_concept("409681000000102", "RxNorm")
        assert isinstance(result, list)

    def test_batch_map_concepts(self):
        """Test batch mapping of multiple concepts."""
        from src.snomed_methods import batch_map_concepts

        result = batch_map_concepts(
    ...
    def test_batch_map_concepts_all_vocabs(self):
        """Test batch mapping with all target vocabularies."""
        from src.snomed_methods import batch_map_concepts

        result = batch_map_concepts(["409681000000102"])
        assert isinstance(result, dict)
...
class TestConfidenceScoring:
    ...
    def test_confidence_calculation_basic(self):
        ...
        confidence = mapper._calculate_confidence(row)
        assert 0.9 <= confidence <= 1.0

    def test_confidence_calculation_missing_correlation(self):
...
class TestICDDescriptions:
    ...
    def test_icd_description_known_code(self):
        """Test getting description for a known ICD code."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        desc = mapper._get_icd_description("C83.3")
        assert len(desc) > 0
        assert "mantle" in desc.lower() or "lymphoma" in desc.lower()

    def test_icd_description_unknown_code(self):
        """Test getting description for an unknown ICD code."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        desc = mapper._get_icd_description("ZZZ99")
        assert len(desc) == 0
...
class TestLOINCDescriptions:
    ...
    def test_loinc_description_known_code(self):
        """Test getting description for a known LOINC code."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        desc = mapper._get_loinc_description("718-7")
        assert len(desc) > 0
        assert "hemoglobin" in desc.lower()

    def test_loinc_description_unknown_code(self):
        """Test getting description for an unknown LOINC code."""
        from src.snomed_methods import ConceptMapper

        mapper = ConceptMapper()
        desc = mapper._get_loinc_description("99999-9")
        assert len(desc) == 0
